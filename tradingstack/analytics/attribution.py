...
def _align(weights_df: pd.DataFrame, rets_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(common_index) == 0 or len(common_cols) == 0:
        raise ValueError("No alignment between weights and returns (index/columns mismatch).")
...
def contribution_by_symbol(
    ...
    summary = pd.DataFrame({"total_contrib": total, "avg_contrib": avg, "hit_rate": hit})
...
def contribution_by_group(
    ...
    for g in set(groups):
        ...
        grouped[g] = contrib[cols].sum(axis=1) if cols else pd.Series(0.0, index=contrib.index)
