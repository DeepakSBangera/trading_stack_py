...
ROOT = pathlib.Path(__file__).resolve().parents[1]  # repo root (…/trading_stack_py)
...
def compare(a_path: str, b_path: str) -> pd.DataFrame:
    a = load_equity(a_path)
    b = load_equity(b_path)
    ma = metrics_from_equity(a)
    mb = metrics_from_equity(b)
    tbl = pd.DataFrame.from_dict({"A_older": ma, "B_newer": mb}, orient="index")
    return tbl[["days", "cagr", "sharpe", "max_dd", "total_ret"]]
...
def main():
    ...
    tbl = compare(args.a, args.b)
    tbl.to_parquet(os.path.join(out_dir, "compare_runs.parquet"), index=True)
    tbl.to_csv(os.path.join(out_dir, "compare_runs.csv"), index=True)

    with pd.option_context("display.max_columns", None, "display.width", 120):
        print("Compare written to", out_dir, "(compare_runs.*)")
        print("Summary (ASCII):")
