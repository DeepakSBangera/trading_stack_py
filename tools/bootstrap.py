...
BASE_DIR = r"F:\Projects\trading_stack_py"
...
readme = f'''# Trading · Pricing · Econometrics — Python-First (Low-Cost)
...
def make_signals(df: pd.DataFrame, params: dict, rule_name: str = "R1_trend_breakout_obv") -> pd.DataFrame:
...
if df.empty:
return pd.DataFrame(columns=["date","open","high","low","close","volume"]).set_index(pd.DatetimeIndex([]))
...
def update_positions(buylist: pd.DataFrame, prices_next_open: pd.Series,
positions_csv: str, portfolio_csv: str, cfg: dict, init_equity: float = 1_000_000.0):
try:
...
pd.DataFrame(all_buys).sort_values(by=["score", "symbol"], ascending=[False, True]).to_csv(buylist_path, index=False)
...
def fit_loglog_elasticity(df: pd.DataFrame, price_col: str, qty_col: str, extra_cols: list[str] = None):
...
def apply_recommendations(df: pd.DataFrame, price_col: str, qty_col: str, cost_col: str,
elasticity_abs: float, pct_band: float = 0.1):
...
out_rows.append({"product_id": pid, "elasticity_abs": eps, "current_price": rec["current"],
"cost_proxy": rec["cost"], "rec_price": rec["recommended"],
"band_lo": rec["band"][0], "band_hi": rec["band"][1]})
...
def arima_forecast(s: pd.Series, order=(1,1,1), steps=12):
s = s.dropna()
...
try:
adf = adf_test(s); kps = kpss_test(s)
...
for d in ["data/prices", "data/features", "db", "reports", "pricing/data", "econo/timeseries", "data/csv"]:
