...
def parse_args():
    ...
    p.add_argument(
        "--tickers", default="", help="Comma-separated tickers, e.g. 'RELIANCE.NS,HDFCBANK.NS,...'"
    )
    p.add_argument(
        "--universe-csv",
        default="",
    ...
    p.add_argument(
        "--lookback", type=int, default=126, help="Lookback days for momentum window (default 126)"
    )
    p.add_argument(
        "--top-n",
        dest="top_n",
    ...
    p.add_argument(
        "--weight-cap", type=float, default=1.0, help="Per-name cap (0–1, default 1.0=no cap)."
    )
    p.add_argument(
        "--min-weight", type=float, default=0.0, help="Drop names below this weight (0–1)."
    )
    ...
    p.add_argument(
        "--benchmark", default="", help="Optional benchmark ticker (e.g., ^NSEI). Empty to disable."
    )
...
def cap_min_normalize(
    weights_series: pd.Series, max_holdings: int, wcap: float, wmin: float, cash_buffer: float
) -> pd.Series:
    """
    Apply:
...
def main():
    ...
    for i, d in enumerate(rb_dates):
        ...
        # Trades log
        block_end_date = (
            rets.index[end_block - 1] if end_block > start_block else rets.index[start_block]
        )
    ...
    if args.benchmark.strip():
        ...
        b_raw = yf.download(
            [bench_label], start=str(eq.index[0].date()), progress=False, auto_adjust=False
        )
